...
import numpy as np
import random
from mnist_wgan_gp_class import *
import time
...
# d_optimizer = optim.Adam(discriminator_d.parameters(), lr=0.0002)  # use Adam 
g_optimizer = optim.RMSprop(generator_d.parameters(), lr=learning_rate)  # use RMSprop optimizer
d_optimizer = optim.RMSprop(discriminator_d.parameters(), lr=learning_rate)  # use RMSprop optimizer
# record training time
start_time = time.time()
train_both_models(generator_d, discriminator_d, train_loader, g_optimizer, d_optimizer, lambda_gp=lambda_gp, num_epochs=num_epochs, step_g=2, step_d=2, device=device)
end_time = time.time()
