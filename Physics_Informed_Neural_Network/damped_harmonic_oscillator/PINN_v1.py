...
In version 1, we don't introduce the PDE loss term in the training process, 
and we only train the network to fit the analytical solution.
...
n_epochs = 10000 # The number of training epochs for classifier. 
...
n_points_training = 5 * batch_size  # Number of data points used for training (N times of batch size)
...
model = FCN(N_hidden=64, dropout_rate=0.1).to(device)
...
plt.legend()
plt.savefig('training_validation_loss_curves_v1.png')
# save the trained model
torch.save(model.state_dict(), f'pinn_v1_trained_model_epoch_{n_epochs}.pth')
end_time = time.time()
...
if best_epoch > 0:
    ...
    best_model.load_state_dict(torch.load(f'pinn_v1_best_model_epoch_{best_epoch}.pth'))
    best_model.eval()
    with torch.no_grad():
