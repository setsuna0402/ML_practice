'''
This is an example of a Physics-Informed Neural Network (PINN) for solving the 1D Radiative Transfer Equation (RTE).
...
Version one: Succeed from PINN_1d_RT_v6.py.
Input is [x, I_o, x_c, frequency], where x is the spatial variable, I_o is the initial intensity,
x_c is the center of the absorption coefficient, and frequency is the frequency of the emission coefficient.
The main difference from PINN_1d_RT_v6.py is the NN architecture. This one introduces a fourier feature layer.
'''
...
from NN_classes import FF_FCN_four
...
model = FF_FCN_four(N_hidden=64, ff_dim=64, sigma=8.0).to(device)
    ...
    if (epoch + 1) % 20 == 0:
        ...
        with torch.no_grad():
            ...
            if (val_loss < best_val_loss and epoch > 100):  # Save the model if it has the best validation loss or every 100 epochs
                ...
                # save the best model
                torch.save(model.state_dict(), output_dir / f'ff_pinn_v1_best_model_epoch_{epoch+1}.pth')
                best_epoch = epoch + 1
            if (epoch + 1) % 100 == 0:
                # save the model every 100 epochs
                torch.save(model.state_dict(), output_dir / f'ff_pinn_v1_model_epoch_{epoch+1}.pth')
        ...
        fig.savefig(output_dir / f'ff_pinn_v1_epoch_{epoch+1}.png')
        plt.close(fig)
        model.train()
endtime = time.time()
