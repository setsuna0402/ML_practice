...
# Dense time samples used for residual evaluation/visualisation.
N_RESIDUAL_PLOT_TIMES = 1024
...
def build_dense_trajectory_data(
    X_value: np.ndarray,
    Y_value: np.ndarray,
    X_pde: np.ndarray,
    PDE_aux: np.ndarray,
    n_times_dense: int = N_RESIDUAL_PLOT_TIMES,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, int]:
    """
    Reconstruct each trajectory on a dense common time grid.

    Value targets are PCHIP-interpolated from the HDF5 /value grid.
    Frozen PDE coefficients k1..k6 are PCHIP-interpolated from the HDF5 /pde grid.

    Returns
    -------
    X_value_dense : (N_traj*n_times_dense, 12)
    Y_value_dense : (N_traj*n_times_dense, 4)
    X_pde_dense   : (N_traj*n_times_dense, 12)
    PDE_aux_dense : (N_traj*n_times_dense, 6)
    n_trajectories
    """
    ...
    for n_traj in range(1, min(n_value_total, n_pde_total) + 1):
        ...
        nv = n_value_total // n_traj
        npde = n_pde_total // n_traj

        if nv == npde + 1:
            candidates.append((n_traj, nv, npde))
    ...
    Xv = X_value.reshape(n_trajectories, n_value_times, -1)
    Yv = Y_value.reshape(n_trajectories, n_value_times, -1)
    Xp = X_pde.reshape(n_trajectories, n_pde_times, -1)
    Ap = PDE_aux.reshape(n_trajectories, n_pde_times, -1)
    ...
    for traj in range(n_trajectories):
        ...
        # Conditioning parameters are constant along one trajectory.
        X_dense = np.repeat(
            Xv[traj, 0:1, :],
            n_times_dense,
            axis=0,
        ...
        # Interpolate Grackle value targets.
        Y_dense = np.empty(
            (n_times_dense, Yv.shape[-1]),
            dtype=Yv.dtype,
        )
        for j in range(Yv.shape[-1]):
            ...
            Y_dense[:, j] = interp(t_dense)

        # Interpolate frozen chemistry coefficients.
        # The original PDE coefficients live at midpoint collocation times.
        A_dense = np.empty(
            (n_times_dense, Ap.shape[-1]),
            dtype=Ap.dtype,
        )
        for j in range(Ap.shape[-1]):
            ...
            A_dense[:, j] = interp(t_dense)

        X_value_dense_all.append(X_dense)
        Y_value_dense_all.append(Y_dense)

        # Same dense conditioning grid is used for PDE diagnostics.
        X_pde_dense_all.append(X_dense.copy())
        PDE_aux_dense_all.append(A_dense)

    return (
        np.concatenate(X_value_dense_all, axis=0),
        np.concatenate(Y_value_dense_all, axis=0),
        np.concatenate(X_pde_dense_all, axis=0),
        np.concatenate(PDE_aux_dense_all, axis=0),
        n_trajectories,
    )
...
def save_trajectory_residual_plots(
    ...
    """
    n_value_times = X_value.shape[0] // n_trajectories
    n_pde_times = X_pde.shape[0] // n_trajectories
...
def main() -> None:
    ...
    (
        ...
        PDE_aux,
        n_trajectories,
    ) = build_dense_trajectory_data(
        X_value,
        Y_value,
        X_pde,
        PDE_aux,
        n_times_dense=N_RESIDUAL_PLOT_TIMES,
    )

    print(f"Checkpoint:       {checkpoint_path}")
    print(f"HDF5 dataset:     {data_path}")
    print(f"Trajectories:      {n_trajectories}")
    print(f"Dense time points: {N_RESIDUAL_PLOT_TIMES}")
    ...
    print_statistics(
        "PDE DERIVATIVE: physical dX/dt",
        derivative_phys,
        PDE_NAMES,
    )

    print_statistics(
        "PDE RHS: physical chemistry RHS",
        rhs_phys,
        PDE_NAMES,
    )
