...
def denormalise_outputs(y):
    ...
    For torch tensors, avoid in-place slice assignment so this remains safe
    for higher-order autograd used by the PINN residual.
    """
    if torch.is_tensor(y):
        HI = _denormalise_fraction(y[..., 0])
        HeI = _denormalise_fraction(y[..., 1])
        HeII = _denormalise_fraction(y[..., 2])
        u = U_REF * _pow10(y[..., 3])
        return torch.stack((HI, HeI, HeII, u), dim=-1)
