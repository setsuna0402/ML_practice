...
- supervised trajectory loss plus v4 physical-fraction PINN residual
- v4, compute the derivative of the normalised quantitties, 
  then transform to physical space to compute the PDE residual.
...
# Gradient clipping limits one optimizer step from being dominated by a
# transiently large PINN gradient.
MAX_GRAD_NORM = 1.0
...
#     x = 10 ** ((f - C) / A)
#
# v4 differentiates f directly, then converts df/dt to the physical-fraction
# derivative through:
#     dx/dt = (ln(10) / A) * x * df/dt
_LOG_FRACTION_FLOOR = np.log10(FRACTION_FLOOR)
FRACTION_LOG_A = (
    (FRACTION_NORM_MAX - FRACTION_NORM_MIN)
    / (0.0 - _LOG_FRACTION_FLOOR)
)
FRACTION_LOG_C = (
    FRACTION_NORM_MIN
    - FRACTION_LOG_A * _LOG_FRACTION_FLOOR
)
FRACTION_CHAIN_FACTOR = np.log(10.0) / FRACTION_LOG_A
loss_weights = {
    "HI": 1.0,
    "HeI": 1.0,
    "HeII": 1.0,
...
def frozen_coefficient_species_loss(model, input_batch, aux_batch, criterion):
    """Semi-consistent physical-fraction residual with frozen Grackle k1..k6.

    v4 keeps the network output and autograd derivative in normalized fraction
    space, but evaluates the chemistry residual in physical fraction space.
    ...
    This avoids differentiating through ``denormalise_outputs`` while also
    avoiding the 1/X terms that appear when the entire chemistry equation is
    rewritten in log-normalized fraction space.

    Reaction coefficients k1..k6 are frozen to the reference Grackle
    trajectory at the midpoint collocation times. Electron density is
    reconstructed self-consistently from the NN-predicted physical fractions.
    """
    ...
    # -------------------------------------------------------------
    # Derivatives of the normalized network outputs.
    # -------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    # Physical species fractions reconstructed algebraically from
    # normalized outputs. No in-place operation is used.
    # -------------------------------------------------------------
    HI = torch.pow(
        ten, (f_HI - FRACTION_LOG_C) / FRACTION_LOG_A
    )
    HeI = torch.pow(
        ten, (f_HeI - FRACTION_LOG_C) / FRACTION_LOG_A
    )
    HeII = torch.pow(
        ten, (f_HeII - FRACTION_LOG_C) / FRACTION_LOG_A
    )
    ...
    # -------------------------------------------------------------
    # Chain rule: convert df/dt to physical-fraction dX/dt.
    # -------------------------------------------------------------
    chain_factor = torch.as_tensor(
        FRACTION_CHAIN_FACTOR,
        dtype=x.dtype,
        device=x.device,
    )
    ...
    # Frozen Grackle rate coefficients.
    k1, k2, k3, k4, k5, k6 = [
        aux_batch[:, i] for i in range(6)
    ]

    # -------------------------------------------------------------
    # Reconstruct Grackle e_density from the NN state.
    #
    # For Grackle's mass-density convention:
    #   e_density =
    #       rho_HII
    #       + 1/4 rho_HeII
    #       + 1/2 rho_HeIII
    ...
    # Recover physical/code-time photoionisation rates from normalized inputs.
    ...
    # -------------------------------------------------------------
    # Physical fraction chemistry equations.
    # -------------------------------------------------------------
    rhs_HI = (
        -(k1 * e_nn + Gamma_HI) * HI
        + k2 * e_nn * HII
    ...
    # v4 deliberately uses raw physical-fraction residuals. There is no
    # species-dependent division by PDE_*_RATE_REF.
    residual_HI = grad_HI - rhs_HI
    residual_HeI = grad_HeI - rhs_HeI
    residual_HeII = grad_HeII - rhs_HeII
