...
so we implement a full-batch training.
...
n_epochs = 100 # The number of training epochs for classifier. 
# Batch size for training, validation, and testing.
# A greater batch size usually gives a more stable gradient.
# But the GPU memory is limited, so please adjust it carefully.
batch_size = 100  # Useless for L-BFGS optimizer, since it doesn't support mini-batch.
...
parser.add_argument("-I", "--checkpoint", type=Path, default="./pinn_v6_best_model_epoch_1340.pth", help="Path to a .pth checkpoint.")
...
parser.add_argument("-O", "--output-dir", type=Path, default=Path("./trained_result_plots_v6_fine_tuning_c"), help="Directory where checkpoints are saved.")
parser.add_argument("-P", "--plot-dir", type=Path, default=Path("./trained_result_plots_v6_fine_tuning_c"), help="Directory where plots are saved.")
...
x_location_pde = torch.linspace(0, Boxsize_training, n_points_pde_loss).view(-1, 1)  # sample locations over the problem domain
...
train_inputs_device = train_inputs.reshape(-1, 4).to(device)
train_targets_device = I_training.reshape(-1, 1).to(device)
...
physics_inputs_device = physics_inputs.reshape(-1, 4).to(device).requires_grad_(True)
physics_inputs_cpu = physics_inputs.reshape(-1, 4).detach().cpu()  # Keep a CPU copy for reference if needed
...
for epoch in tqdm(range(n_epochs), desc="Training", disable=run_in_background):
    def closure() -> torch.Tensor:
        optimizer.zero_grad()
        I_pred = model(train_inputs_device)
        loss_data = criterion(I_pred, train_targets_device)
        # alpha = min(1.0, epoch / 250) * alpha_phys # slowly increase the weight of the physics loss term
        alpha = alpha_phys # slowly increase the weight of the physics loss term
        ...
        kappa_val = kappa_func(
            physics_inputs_cpu[:, :1],
            physics_inputs_cpu[:, 2:3],
        ).to(device)
        j_emit_val = j_emit_func(
            physics_inputs_cpu[:, :1],
            physics_inputs_cpu[:, 3:4],
        ).to(device)
    ...
    epoch_loss = float(loss_value.item()) if torch.is_tensor(loss_value) else float(loss_value)
    ...
    if (epoch + 1) % 1 == 0:
        ...
        with torch.no_grad():
            ...
            if val_loss < best_val_loss and epoch >= 0:
                ...
                # save the best model
                torch.save(model.state_dict(), output_dir / f'pinn_v6_best_model_epoch_{epoch+1}.pth')
                best_epoch = epoch + 1
            if (epoch + 1) % 5 == 0:
                best_val_loss = val_loss
                # save the best model
                torch.save(model.state_dict(), output_dir / f'pinn_v6_model_epoch_{epoch+1}.pth')
        ...
        fig.savefig(plot_dir / f'pinn_v6_epoch_{epoch+1}.png')
        plt.close(fig)
        model.train()
endtime = time.time()
