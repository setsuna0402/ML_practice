...
def I_spline_callable(x: torch.Tensor) -> torch.Tensor:
    ...
    I_o : torch.Tensor
        Initial intensity. Can be a single value or a batch of values.
    Returns:
    torch.Tensor
        Intensity at location x. Same shape as input x.
    '''
    x_np = x.detach().cpu().numpy()  # Convert to numpy array
    I_x_np = I_spline(x_np)  # Evaluate the cubic spline at location x with the corresponding I_o
    I_x = torch.from_numpy(I_x_np).to(device=x.device, dtype=torch.float32)  # Convert back to torch tensor in float32
    return I_x


def reference_intensity_from_I_o(x_values: torch.Tensor, I_o_value: torch.Tensor) -> torch.Tensor:
    """
    Evaluate the reference intensity for one initial condition value.
    This keeps the target-generation logic separate from the model-facing helper.
    """
    I_o_scalar = float(I_o_value.detach().cpu().reshape(-1)[0].item())
...
def build_pair_dataset(x_values: torch.Tensor, I_o_values: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Build a Cartesian-product dataset for the supervised loss.
    Each input row is [x, I_o], and the target is I(x; I_o).
    """
    x_column = x_values.view(-1, 1, 1) #[N, 1] => [N, 1, 1]
    I_o_column = I_o_values.view(1, -1, 1) # [M, 1] => [1, M, 1]

    x_grid = x_column.expand(-1, I_o_column.shape[1], -1) # [N, 1, 1] => [N, M, 1]
    I_o_grid = I_o_column.expand(x_column.shape[0], -1, -1) # [1, M, 1] => [N, M, 1]
    input_grid = torch.cat([x_grid, I_o_grid], dim=2) # [N, M, 1] + [N, M, 1] => [N, M, 2]

    target_grid = []
    for I_o_value in I_o_values.view(-1):
        target_grid.append(reference_intensity_from_I_o(x_values, I_o_value).view(-1, 1))
...
# Create I_o linearly spaced between 0.1 and 1.0 for the training data
...
train_inputs, I_training = build_pair_dataset(x_location_training, I_o_training)  # Numerical solution at the training position points
truth_inputs, I_truth = build_pair_dataset(x_location_truth, I_o_training)  # Numerical solution over the full truth grid

x_location_pde = torch.linspace(0, 2*Boxsize_training, n_points_pde_loss).view(-1, 1)  # sample locations over the problem domain
physics_inputs, _ = build_pair_dataset(x_location_pde, I_o_training)
...
I_validate = I_spline_callable(x_location_validate).view(-1, 1)  # numerical solution at the validation position points
validate_inputs = torch.cat([x_location_validate, torch.full_like(x_location_validate, validate_I_o.item())], dim=1)


# check array shapes
print(f"truth_inputs shape: {truth_inputs.shape}, I_truth shape: {I_truth.shape}")
...
train_dataset = torch.utils.data.TensorDataset(train_inputs.reshape(-1, 2), I_training.reshape(-1, 1))
...
physics_inputs_device = physics_inputs.reshape(-1, 2).to(device).requires_grad_(True)
...
for epoch in tqdm(range(n_epochs), desc="Training", disable=run_in_background):
    ...
    for input_batch, I_batch in train_dataloader:
        ...
        I_val = model(physics_inputs_device)
        dx  = torch.autograd.grad(I_val, physics_inputs_device, torch.ones_like(I_val), create_graph=True)[0][:, :1] # computes dI/dx
        kappa_val = kappa_func(physics_inputs_device[:, :1])
        j_emit_val = j_emit_func(physics_inputs_device[:, :1])
    ...
    if (epoch + 1) % 200 == 0:
        model.eval()
        with torch.no_grad():
            validate_inputs_device, I_validate = validate_inputs.to(device), I_validate.to(device)
            ...
            # validate_I_o = torch.tensor([[1.0]], dtype=torch.float32)
            I_numerical = I_spline_callable(x_location_truth).view(-1, 1)  # numerical solution at the validation position points
            model_inputs = torch.cat([x_location_truth, torch.full_like(x_location_truth, validate_I_o.item())], dim=1)
            I_model = model(model_inputs.to(device)).cpu()  # Model prediction for the numerical position points
            train_inputs = torch.cat([x_location_training, torch.full_like(x_location_training, validate_I_o.item())], dim=1)
            I_training_pred = model(train_inputs.to(device)).cpu()  # Model prediction for the training position points

        print(f"Epoch {epoch+1}, Validation Loss: {val_loss:.6f}")
        plt.figure(figsize=(10, 6))
        plt.plot(x_location_truth.cpu(), I_numerical.cpu(), '-b', label='Numerical Solution')
        plt.plot(x_location_truth.cpu(), I_model, '--r', label='Model Prediction')
        plt.plot(x_location_training.cpu(), I_training_pred.cpu(), 'go', label='Training Data')
