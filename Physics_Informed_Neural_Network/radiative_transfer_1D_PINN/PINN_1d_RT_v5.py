...
parser.add_argument("-O", "--output-dir", type=Path, default=Path("./trained_result_plots_v5"), help="Directory where checkpoints and plots are saved.")
    ...
    if (epoch + 1) % 20 == 0:
        ...
        with torch.no_grad():
            ...
            if (val_loss < best_val_loss and epoch > 100):  # Save the model if it has the best validation loss or every 100 epochs
                ...
                # save the best model
                torch.save(model.state_dict(), output_dir / f'pinn_v5_best_model_epoch_{epoch+1}.pth')
                best_epoch = epoch + 1
            if (epoch + 1) % 100 == 0:
                # save the model every 100 epochs
                torch.save(model.state_dict(), output_dir / f'pinn_v5_model_epoch_{epoch+1}.pth')
        ...
        fig.savefig(output_dir / f'pinn_v5_epoch_{epoch+1}.png')
        plt.close(fig)
        model.train()
endtime = time.time()
