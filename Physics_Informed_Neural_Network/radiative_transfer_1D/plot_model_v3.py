...
from NN_classes import FCN_four, FCN_four_short
...
def load_model(checkpoint_path: Path, device: torch.device):
	model = FCN_four(N_hidden=64, dropout_rate=0.0).to(device)
...
def save_curve_plot(output_dir: Path, x_values: torch.Tensor, reference: torch.Tensor, prediction: torch.Tensor, title: str, filename: str) -> None:
	...
	ax.axvspan(0.0, Boxsize_training, color="tab:green", alpha=0.08, label="Training (value) x-range")
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument("-O", "--output-dir", type=Path, default=Path("trained_result_plots_v3_b"), help="Directory where plots are saved.")
...
def main() -> None:
	...
	'''
	sweep_specs = [
		("I_o", torch.linspace(0.1, 1.0, 6).view(-1, 1), torch.tensor([[0.2]], dtype=torch.float32), torch.tensor([[1.0]], dtype=torch.float32)),
		("x_c", torch.linspace(0.0, 0.9, 6).view(-1, 1), torch.tensor([[1.0]], dtype=torch.float32), torch.tensor([[1.0]], dtype=torch.float32)),
		("frequency", torch.linspace(0.05, 1.05, 6).view(-1, 1), torch.tensor([[1.0]], dtype=torch.float32), torch.tensor([[0.2]], dtype=torch.float32)),
	]

	sweep_output_dir = output_dir / "condition_sweeps"
	sweep_output_dir.mkdir(parents=True, exist_ok=True)

	for sweep_name, sweep_values, fixed_one, fixed_two in sweep_specs:
		for index, value in enumerate(sweep_values.view(-1)):
			if sweep_name == "I_o":
				I_o_value = value.view(1, 1)
				x_c_value = fixed_one
				freq_value = fixed_two
			elif sweep_name == "x_c":
				I_o_value = fixed_one
				x_c_value = value.view(1, 1)
				freq_value = fixed_two
			else:
				I_o_value = fixed_one
				x_c_value = fixed_two
				freq_value = value.view(1, 1)
			...
			title = (
				f"PINN v3 sweep: {sweep_name}, "
				f"I_o = {_to_scalar(I_o_value):.2f}, x_c = {_to_scalar(x_c_value):.2f}, "
				f"frequency = {_to_scalar(freq_value + 0.45):.2f} ({checkpoint_label})"
			)
			filename = f"pinn_v3_{sweep_name}_sweep_{index:02d}.png"
			save_curve_plot(sweep_output_dir, x_values, reference, prediction, title, filename)
	'''
	I_o = torch.linspace(0.15, 1.0, 6).view(-1, 1)
	x_c = torch.linspace(0.05, 0.9, 6).view(-1, 1)
	freq = torch.linspace(0.09, 1.05, 6).view(-1, 1)
	sweep_output_dir = output_dir / "condition_sweeps"
	sweep_output_dir.mkdir(parents=True, exist_ok=True)
