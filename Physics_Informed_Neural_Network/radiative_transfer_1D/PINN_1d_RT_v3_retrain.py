...
n_points_value_loss = 100  # Number of data points for the value loss term
...
parser.add_argument("-P", "--plot-dir", type=Path, default=Path("./trained_result_plots_v3_retrain_b"), help="Directory where plots are saved.")
    ...
    if (epoch + 1) % 20 == 0:
        ...
        with torch.no_grad():
            ...
            if val_loss < best_val_loss and epoch >= 0:
                ...
                # save the best model
                torch.save(model.state_dict(), output_dir / f'pinn_v3_retrain_best_model_epoch_{epoch+1}.pth')
                best_epoch = epoch + 1
        ...
        fig.savefig(plot_dir / f'pinn_v3_retrain_epoch_{epoch+1}.png')
        plt.close(fig)
        model.train()
endtime = time.time()
