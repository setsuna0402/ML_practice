...
parser.add_argument("-O", "--output-dir", type=Path, default=Path("./trained_result_plots_v6_b"), help="Directory where checkpoints and plots are saved.")
...
for epoch in tqdm(range(n_epochs), desc="Training", disable=run_in_background):
    ...
    if (epoch + 1) % 20 == 0:
        ...
        fig.savefig(output_dir / f'pinn_v6_epoch_{epoch+1}.png')
        plt.close(fig)
        model.train()
endtime = time.time()
