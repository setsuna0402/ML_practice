...
n_epochs = 2000 # The number of training epochs for classifier. 
...
parser.add_argument("-O", "--output-dir", type=Path, default=Path("./trained_result_plots_v3_big"), help="Directory where checkpoints and plots are saved.")
...
model = FCN_four(N_hidden=96, dropout_rate=0.0).to(device)
...
print(physics_inputs_cpu[:, :1])
print(physics_inputs_cpu[:, :1].shape)
print(physics_inputs_cpu.shape)
print(physics_inputs.shape)
exit()
    ...
    if (epoch + 1) % 20 == 0:
        ...
        with torch.no_grad():
            ...
            if val_loss < best_val_loss and epoch >= 100:
                ...
                # save the best model
                torch.save(model.state_dict(), output_dir / f'pinn_v3_best_model_epoch_{epoch+1}.pth')
                best_epoch = epoch + 1
            if (epoch + 1) % 100 == 0:
                # save the model every 100 epochs
                torch.save(model.state_dict(), output_dir / f'pinn_v3_model_epoch_{epoch+1}.pth')
        ...
        fig.savefig(output_dir / f'pinn_v3_epoch_{epoch+1}.png')
        plt.close(fig)
        model.train()
endtime = time.time()
