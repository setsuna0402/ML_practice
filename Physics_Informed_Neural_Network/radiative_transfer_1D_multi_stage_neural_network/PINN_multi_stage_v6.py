...
Version 5: Use sine-tanh activation function for the tiny network with hard constraint to predict the residual.

Version 6: Try LeakRELU activation function for the tiny network with hard constraint to predict the residual.
'''
...
from NN_classes import FCN_hard_bc_four, FCN_tiny_hard_constraint, FCN_tiny_LeakRELU
...
n_epochs = 500 # The number of training epochs for classifier. 
...
# omega_hidden=10.0 # For the hidden sine layers of the second stage network.
...
parser.add_argument("-I", "--checkpoint", type=Path, default=("./pinn_v7_best_model_epoch_960.pth"), help="Path to a .pth checkpoint.")
parser.add_argument("-O", "--output-dir", type=Path, default=Path("./trained_result_plots_v6_a"), help="Directory where checkpoints and plots are saved.")
...
stage_b_model = FCN_tiny_LeakRELU(N_hidden=64, kappa=initial_scaling_factor).to(device)
    ...
    if (epoch + 1) % 20 == 0:
        ...
        with torch.no_grad():
            ...
            if (val_loss < best_val_loss and epoch > 50):  # Save the model if it has the best validation loss or every 100 epochs
                ...
                # save the best model
                torch.save(save_dict, output_dir / f'multi_stage_v6_best_model_epoch_{epoch+1}.pth')
                best_epoch = epoch + 1
            if (epoch + 1) % 100 == 0:
                save_dict = {
                    'model_state_dict': stage_b_model.state_dict(),
                ...
                torch.save(save_dict, output_dir / f'multi_stage_v6_model_epoch_{epoch+1}.pth')
        ...
        fig.savefig(output_dir / f'multi_stage_v6_residual_epoch_{epoch+1}.png')
        plt.close(fig)
        stage_b_model.train()
endtime = time.time()
