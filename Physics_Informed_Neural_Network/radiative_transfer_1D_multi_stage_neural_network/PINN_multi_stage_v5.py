...
Version 4: Use sine-tanh activation function for the tiny network with hard constraint to predict the residual.
'''
...
from NN_classes import FCN_hard_bc_four, FCN_tiny_hard_constraint, FCN_tiny_sine_tanh_hard_constraint_b
...
parser.add_argument("-O", "--output-dir", type=Path, default=Path("./trained_result_plots_v5_a"), help="Directory where checkpoints and plots are saved.")
...
stage_b_model = FCN_tiny_sine_tanh_hard_constraint_b(N_hidden=64, omega_first=omega_first).to(device)
    ...
    if (epoch + 1) % 20 == 0:
        ...
        with torch.no_grad():
            ...
            if (val_loss < best_val_loss and epoch > 50):  # Save the model if it has the best validation loss or every 100 epochs
                ...
                # save the best model
                torch.save(save_dict, output_dir / f'multi_stage_v5_best_model_epoch_{epoch+1}.pth')
                best_epoch = epoch + 1
            if (epoch + 1) % 100 == 0:
                save_dict = {
                    'model_state_dict': stage_b_model.state_dict(),
                ...
                torch.save(save_dict, output_dir / f'multi_stage_v5_model_epoch_{epoch+1}.pth')
        ...
        fig.savefig(output_dir / f'multi_stage_v5_residual_epoch_{epoch+1}.png')
        plt.close(fig)
        stage_b_model.train()
endtime = time.time()
