...
from NN_classes import FCN_hard_bc_four, FCN_tiny, FF_FCN_four
...
parser.add_argument("-O", "--output-dir", type=Path, default=Path("./trained_result_plots_v3_a"), help="Directory where checkpoints and plots are saved.")
...
stage_b_model = FF_FCN_four(N_hidden=64).to(device)
...
for epoch in tqdm(range(n_epochs), desc="Training", disable=run_in_background):
    ...
    for input_batch, I_batch in train_dataloader:
        ...
        kappa_val = kappa_func(physics_batch[:, :1], physics_batch[:, 2:3])

        pde = dx + kappa_val * residual_pred_physics
    ...
    if (epoch + 1) % 20 == 0:
        ...
        with torch.no_grad():
            ...
            if (val_loss < best_val_loss and epoch > 50):  # Save the model if it has the best validation loss or every 100 epochs
                ...
                # save the best model
                torch.save(save_dict, output_dir / f'multi_stage_v3_best_model_epoch_{epoch+1}.pth')
                best_epoch = epoch + 1
            if (epoch + 1) % 100 == 0:
                save_dict = {
                    'model_state_dict': stage_b_model.state_dict(),
                ...
                torch.save(save_dict, output_dir / f'multi_stage_v3_model_epoch_{epoch+1}.pth')
        ...
        fig.savefig(output_dir / f'multi_stage_v3_residual_epoch_{epoch+1}.png')
        plt.close(fig)
        stage_b_model.train()
endtime = time.time()
