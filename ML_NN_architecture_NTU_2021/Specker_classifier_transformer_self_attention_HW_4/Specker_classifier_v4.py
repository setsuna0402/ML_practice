...
In the version 0.4, we implenment conformer.
...
from Model_Class import Classifier_Conformer_AMS_SAP_MultiHead
...
n_save_step = 10000 # The number of steps for saving the model. 
ratio_train = 0.9 # 90% of the dataset are allocated into training set. 10% of them go to validation set. 
ams_m = 0.25 # Margin for AM-Softmax. 
ams_s = 30.0 # Scale for AM-Softmax. 
n_transformer_layer = 2 # The number of transformer layers in the classifier. 
n_head_SAP = 5 # The number of heads in self-attentive pooling. 
lambda_reg = 1.e-2 # hyperparameter for the loss of the self-attentive pooling. 
kernal_size = 15 # The kernel size for depthwise convolution in conformer.
...
model = Classifier_Conformer_AMS_SAP_MultiHead(n_spks=speaker_num, am_m=ams_m, am_s=ams_s, n_transformer_layer=n_transformer_layer, n_head=n_head_SAP, kernel_size=kernal_size).to(device)
...
for step in range(n_total_step):
    ...
    # Calculate loss
    logits, reg_sap = model(mels, labels=labels) # sharp: [batch, speaker_num], scalar. labels!=None, so the margin in AM-Softmax is enabled.
    loss = criterion(logits, labels) + lambda_reg * reg_sap # reg_sap: regularization term (Frobenius norm) for self-attentive pooling.
    ...
    if (step + 1) % n_valid_step == 0:
        ...
        with torch.no_grad():
            for batch in tqdm(valid_loader, desc="Valid", ncols=0, unit=" batch", disable=run_in_background):
                ...
                mels = mels.to(device)
                labels = labels.to(device)
                logits, _ = model(mels, labels=None) # During validation, we set labels to None to disable the margin in AM-Softmax.
    ...
    if (step + 1) % n_save_step == 0 and do_save_model:
        if best_state_dict is not None:
            torch.save(best_state_dict, f"specker_classifier_v4_{n_transformer_layer}_layer_total_step_{n_total_step}_step_{step+1}_acc_{best_accuracy:.5f}_last.pth")
pbar.close() 
end_time = time.time()
...
plt.xlabel("Step")
plt.ylabel("Acc")
plt.savefig("specker_classifier_v4_{}_layer_total_step_{}_last.png".format(n_transformer_layer, n_total_step))
plt.close()
