...
In v2, we separate the full convulotional layers into depthwise, pointwise grouped convolution to further reduce the number of parameters.
We also include the residual connection. 
...
from Student_Model_Class import StudentNet_DPC, StudentNet_DPGC, StudentNet_DPGC_Residual, StudentNet_DPGC_Residual_Transformer
...
n_epochs = 600 # The number of training epochs for classifier. 
n_adamw_epochs = 600 # The number of epochs to use AdamW optimizer. We use AdamW for the first n_adamw_epochs epochs and then switch to SGD for stable convergence. 
do_semi = True # Whether to do semi-supervised learning.
# n_resample_pseudo = 5 # Redo pseudo labelling every n_resample_pseudo epochs
# n_threshold = 80 # Start to do semi-supervise after n_threshold epochs
...
temperature = 20.0 # The temperature for softening the probability distributions in knowledge distillation
...
# print(f"n_resample_pseudo: {n_resample_pseudo}")
...
student_model = StudentNet_DPGC_Residual()
...
for epoch in range(n_epochs):
    ...
    # Update the learning rate if we are in SGD stage.
    if n_adamw_epochs <= epoch < n_epochs:
    ...
    if valid_acc > best_valid_acc and epoch >= 80: # Save the model with best validation accuracy after epoch 80. 
        ...
        torch.save(model_dict, "nn_compression_v2_batch_{}_epoch_{}_T_{}_alpha_{}_CutMix_semi_supervised_AdamW.pth".format(batch_size, epoch, temperature, alpha))
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.savefig("nn_compression_v2_accuracy_batch_{}_epoch_{}_T_{}_alpha_{}_CutMix_semi_supervised_AdamW.png".format(batch_size, n_epochs, temperature, alpha))
plt.close()
...
torch.save(model_dict, "nn_compression_v2_batch_{}_epoch_{}_T_{}_alpha_{}_CutMix_semi_supervised_AdamW.pth".format(batch_size, n_epochs, temperature, alpha))
print("Model saved.")
