...
V3 is similar to v2, but we further include the transformer block in the student model.
...
n_adamw_epochs = 160 # The number of epochs to use AdamW optimizer. We use AdamW for the first n_adamw_epochs epochs and then switch to SGD for stable convergence. 
do_semi = True # Whether to do semi-supervised learning.
n_resample_pseudo = 5 # Resampling pseudo data every n_resample_pseudo epochs
# n_threshold = 20 # Start to do semi-supervise after n_threshold epochs
...
student_model = StudentNet_DPGC_Residual_Transformer()
...
optimizer_SGD = torch.optim.SGD(student_model.parameters(), lr=0.0003, momentum=0.9, weight_decay=1e-5)
...
for epoch in range(n_epochs):
    ...
    if valid_acc > best_valid_acc and epoch >= 80: # Save the model with best validation accuracy after epoch 80. 
        ...
        torch.save(model_dict, "nn_compression_v3_batch_{}_epoch_{}_T_{}_semi_supervised_AdamW_SGD.pth".format(batch_size, epoch, temperature))
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.savefig("nn_compression_v2_accuracy_batch_{}_epoch_{}_T_{}_semi_supervised_AdamW_SGD.png".format(batch_size, n_epochs, temperature))
plt.close()
...
torch.save(model_dict, "nn_compression_v2_batch_{}_epoch_{}_T_{}_semi_supervised_AdamW_SGD.pth".format(batch_size, n_epochs, temperature))
print("Model saved.")
